...
extern AssimpYinModel *gAssimpYinModel;  // 声明一个指向Assimp模型对象的全局指针，用于不同的本地方法中。
...
Java_com_anandmuralidhar_assimpandroid_GestureClass_DoubleTapNative(JNIEnv *env, jobject instance) {
    ...
    if (gAssimpYinModel != nullptr) {
        gAssimpYinModel->DoubleTapAction();  // 调用Assimp对象的双击动作。
    }
}
...
Java_com_anandmuralidhar_assimpandroid_GestureClass_ScrollNative(
    ...
    if (gAssimpYinModel != nullptr) {
        float dX = distanceX / gAssimpYinModel->GetScreenWidth();
        float dY = -distanceY / gAssimpYinModel->GetScreenHeight();
        float posX = 2 * positionX / gAssimpYinModel->GetScreenWidth() - 1.;
        float posY = -2 * positionY / gAssimpYinModel->GetScreenHeight() + 1.;
        // 将位置限制在[-1, 1]范围内，确保它们在GL视口内
        posX = fmax(-1., fmin(1., posX));
        posY = fmax(-1., fmin(1., posY));
        gAssimpYinModel->ScrollAction(dX, dY, posX, posY);  // 将标准化的值传递给模型的滚动动作方法。
...
Java_com_anandmuralidhar_assimpandroid_GestureClass_ScaleNative(
    ...
    if (gAssimpYinModel != nullptr) {
        gAssimpYinModel->ScaleAction((float) scaleFactor);  // 将缩放因子传递给模型的缩放动作方法。
    }
}
...
Java_com_anandmuralidhar_assimpandroid_GestureClass_MoveNative(
    ...
    if (gAssimpYinModel != nullptr) {
        ...
        float dX = distanceX / gAssimpYinModel->GetScreenWidth();
        float dY = -distanceY / gAssimpYinModel->GetScreenHeight();
        gAssimpYinModel->MoveAction(dX, dY);  // 将标准化的移动距离传递给模型的移动动作方法。
    }
}
